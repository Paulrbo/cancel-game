...
function showConnectionError(err) {
  ...
  const code = err && err.code ? err.code : (err && err.message) || 'inconnue';
  banner.textContent = `⚠️ Connexion refusée par Firebase (${code}). Vérifie les règles de ta Realtime Database (voir le README) — elles expirent après 30 jours en mode test.`;
...
function onRoomUpdate(snapshot) {
  ...
  switch (data.state) {
    ...
    case 'playing':
      ...
      if (isHost && data.roundStartTime) {
        const elapsed = Date.now() - data.roundStartTime;
        if (elapsed > ROUND_DURATION_MS + 2000 && revealTriggeredForRound !== data.currentRoundIndex) {
          triggerReveal(data);
        }
      }
      break;
...
async function triggerReveal(data) {
  ...
  try {
    ...
    let item = poolData.find(p => p.nom === nom);
    if (!item) {
      console.error('Personnalité introuvable dans data.json pour ce round :', nom);
      item = { nom: nom || 'Inconnu', milieu: '', cancel: 'non', categorie: '', raison: "Cette entrée n'a pas pu être retrouvée dans la base." };
    }
    ...
    console.error('Erreur pendant la révélation, on force le passage au round suivant :', err);
    showConnectionError(err);
    revealTriggeredForRound = -1; // permet de retenter
    try {
      await db.ref('rooms/' + roomId).update({ state: 'reveal', lastRoundScores: {}, lastRoundAnswers: {} });
    } catch (err2) {
      console.error('Deuxième tentative échouée aussi :', err2);
    }
  }
}
...
function showRevealUI(data) {
  ...
  ['VSS', 'pedocriminalite', 'delit/crime', 'racisme', 'clean'].forEach(key => {
    ...
    const countKey = key === 'delit/crime' ? 'delit_crime' : key;
